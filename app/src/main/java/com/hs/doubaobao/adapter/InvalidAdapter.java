 ...
 * 邮箱：820159571@qq.com
 *
 * @describe:无效列表的适配器(写多个重复的adapter是为了避免点击冲突，这种情况待优化)
 */

public class InvalidAdapter extends RecyclerView.Adapter {

    private final Context context;
    private List<ListBean> mList;
    private int type = 0;

    public InvalidAdapter(Context context, List<ListBean> mList) {
        this.context = context;
        this.mList = mList;
    }

    public InvalidAdapter(Context context, List<ListBean> mList, int type) {
        this.context = context;
        this.mList = mList;
        this.type = type;
    }
